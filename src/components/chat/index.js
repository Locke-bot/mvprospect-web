...
import { DOC_WS_API as API, refreshToken } from "../api";
...
function Chat() {
  ...
  useEffect(() => {
    if (lastMessage !== null) {
      ...
      if (JSON.parse(lastMessage?.data)?.message === "DONE") {
        ...
        dispatch(fetchPlayerHistoryPreview());
        setControlsText("");
      }
  ...
  useEffect(() => {
    if (!(!player || controlsText === "Stop generating") && inputRef.current) {
      setTimeout(() => {
        if (inputRef.current) {
  ...
  const send = () => {
    setRecording(false);
    if (question.trim() && player && controlsText !== "Stop generating") {
                ...
                <Box className={classes.question}>
                  ...
                  >
                    ...
                    >
                      ...
                      />
                      <a
                        href={`mailto:?subject=TXRChat Text&body=${answers[index]}`}
                        target="_blank"
